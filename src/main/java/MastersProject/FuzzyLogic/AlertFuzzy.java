...
public class AlertFuzzy {
	...
	public AlertFuzzy(){
		...
		alertOutput.addTerm(new Triangle("SOON", 15.000, 67.000, 120.000));
		alertOutput.addTerm(new Triangle("LATER", 60.000, 750.000, 1440.000));
		alertOutput.addTerm(new Triangle("MUCHLATER", 1000.000, 2880.000, 2880.000));
		engine.addOutputVariable(alertOutput);
		
		RuleBlock ruleBlock = new RuleBlock();
		ruleBlock.addRule(Rule.parse("if SenderInput is NIP and SubjectInput is NIP then AlertOutput is MUCHLATER", engine));
		ruleBlock.addRule(Rule.parse("if SenderInput is NIP and SubjectInput is IMPORTANT then AlertOutput is SOON", engine));
		ruleBlock.addRule(Rule.parse("if SenderInput is NIP and SubjectInput is VIP then AlertOutput is NOW", engine));
		
		ruleBlock.addRule(Rule.parse("if SenderInput is IMPORTANT and SubjectInput is NIP then AlertOutput is LATER", engine));
		ruleBlock.addRule(Rule.parse("if SenderInput is IMPORTANT and SubjectInput is IMPORTANT then AlertOutput is VERYSOON", engine));
		ruleBlock.addRule(Rule.parse("if SenderInput is IMPORTANT and SubjectInput is VIP then AlertOutput is NOW", engine));

		ruleBlock.addRule(Rule.parse("if SenderInput is VIP and SubjectInput is NIP then AlertOutput is VERYSOON", engine));
		ruleBlock.addRule(Rule.parse("if SenderInput is VIP and SubjectInput is IMPORTANT then AlertOutput is NOW", engine));
		ruleBlock.addRule(Rule.parse("if SenderInput is VIP and SubjectInput is VIP then AlertOutput is NOW", engine));
	...
	public double processalert(double sender, double subject){
